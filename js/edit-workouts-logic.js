...
let programEditorContext = {}; // For modals inside program editor
...
const addWorkoutToWeekOverlay = document.getElementById('add-workout-to-week-overlay');
...
function renderActiveProgramStatus() {
    ...
    const currentProgramWeek = (activeProgram.startWeekIndex) + weeksPassed + 1;
...
// --- OPTIONS MENU LOGIC (CORRECTED WITH ICONS) ---
...
function openOptionsMenu(button) {
    ...
    if (type === 'workout') {
        ...
        menuItems = `
            <button class="menu-item" data-action="run"><span>Run Program</span></button>
            <button class="menu-item" data-action="share">
                <svg class="share-icon-menu"><use xlink:href="#icon-share"></use></svg>
                <span>Share</span>
            </button>
        ...
        `;
    } else if (type === 'program-workout') {
         menuItems = `
            <button class="menu-item" data-action="edit">
                <svg class="edit-icon-menu"><use xlink:href="#icon-edit-03"></use></svg>
                <span>Edit</span>
            </button>
            <button class="menu-item delete-item" data-action="remove">
                <svg class="delete-icon-menu"><use xlink:href="#icon-cancel-circle-sharp"></use></svg>
                <span>Remove from Week</span>
            </button>
        `;
    }
    ...
    menu.addEventListener('click', (e) => {
        ...
        if (type === 'workout') {
            ...
            const { programName, weekIndex, workoutName } = button.dataset;
            if (action === 'edit') {
                openEditor({ type: 'program', programName, weekIndex: parseInt(weekIndex), workoutName });
            } else if (action === 'remove') {
                if (confirm(`Are you sure you want to remove "${workoutName}" from this week?`)) {
                    delete programs[programName].weeks[weekIndex].workouts[workoutName];
                    const weekBlock = document.querySelector(`.program-week-block[data-week-index='${weekIndex}']`);
                    if(weekBlock) {
                        const newWeekBlock = createWeekBlock(programs[programName].weeks[weekIndex], parseInt(weekIndex));
                        weekBlock.parentNode.replaceChild(newWeekBlock, weekBlock);
                        newWeekBlock.querySelector('.week-content-collapsible').style.maxHeight = newWeekBlock.querySelector('.week-content-collapsible').scrollHeight + "px";
                        newWeekBlock.dataset.state = 'open';
                    }
                }
            }
        }
...
document.addEventListener('click', (e) => {
    ...
    if (optionsBtn) {
        if (optionsBtn.dataset.menuActive === 'true') {
            closeOptionsMenu();
        } else {
    ...
    const activeMenu = document.getElementById('active-options-menu');
...
function openEditor(context) {
    ...
    try {
        if (context.type === 'workout') {
            ...
            title = `Editing: ${context.workoutName} (${programs[context.programName].weeks[context.weekIndex].weekLabel})`;
        }
...
function createWeekBlock(weekData, index) {
    ...
    if (weekData.workouts) {
        Object.keys(weekData.workouts).forEach(workoutName => {
            workoutsHTML += createWeekWorkoutItemHTML(programName, index, workoutName);
        });
    }

    div.innerHTML = `
        <div class="program-week-header program-week-header-clickable">
            <div class="week-label-group">
                <span class="week-label-title">${weekData.weekLabel || `Week ${index + 1}`}</span>
            </div>
            <div class="week-header-controls">
                <button class="action-btn btn-style-90 matt-black-bkg change-duration-btn">
                    <span class="btn-text">Change Duration</span>
                </button>
                <button class="icon-btn delete-week-btn" title="Delete Week">
                    <svg style="fill: var(--system-red);"><use xlink:href="#icon-close"></use></svg>
                </button>
            </div>
        </div>
        <div class="week-content-collapsible">
            <div class="program-week-workouts-list">${workoutsHTML}</div>
            <button class="action-btn add-workout-to-week-btn white-btn">Add Workout to Week</button>
        </div>
    `;
    return div;
}

function createWeekWorkoutItemHTML(programName, weekIndex, workoutName) {
    return `
        <div class="workout-item glass-card week-workout-item" data-workout-name="${workoutName}">
            <span class="workout-item-name">${workoutName}</span>
            <div class="workout-item-actions">
                <button class="icon-btn options-menu-btn" 
                        data-type="program-workout" 
                        data-program-name="${programName}" 
                        data-week-index="${weekIndex}" 
                        data-workout-name="${workoutName}" 
                        title="Options">
                    <svg><use xlink:href="#icon-ellipsis-vertical"></use></svg>
                </button>
            </div>
...
async function saveProgram() {
    ...
    if (oldProgramName && oldProgramName !== newProgramName) {
        programs[newProgramName] = programs[oldProgramName];
        programs[newProgramName].name = newProgramName;
...
function parseWeekLabel(label) {
    const parts = label.replace('Week ', '').split('-').map(Number);
...
async function checkActiveProgram(forceUpdate = false) {
    ...
    for(let i = 0; i < programBlueprint.weeks.length; i++) {
        const range = parseWeekLabel(programBlueprint.weeks[i].weekLabel);
        if(currentProgramWeek >= range.start && currentProgramWeek <= range.end) {
            targetWeekIndex = i;
            break;
        }
    }
    if (targetWeekIndex === -1) targetWeekIndex = programBlueprint.weeks.length - 1; // Default to last week if out of range
    ...
    if (needsDbUpdate) {
        ...
        else showToast(`Program updated to Week ${currentProgramWeek}!`, 'success');
    }
}
...
function getProgramWorkoutAiPrompt(programName, weekLabel, workoutName) {
...
function getFullProgramAiPrompt() {
    return `You are an expert fitness assistant. Your task is to analyze the entire provided user file and transcribe it into a single, structured JSON object representing a complete multi-week program.

Your entire response MUST be only the JSON object. Do not include any text, explanations, or markdown formatting like \`\`\`json outside of the JSON object.

The root JSON object MUST have two keys:
1.  "name": A string for the program's title. Infer this from the document.
2.  "weeks": An array of week-block objects.

Each object inside the "weeks" array represents a block of training and MUST have two keys:
1.  "weekLabel": A string describing the week or range (e.g., "Week 1", "Weeks 2-5", "Week 6: Deload").
2.  "workouts": An object where each key is the name of a workout for that week (e.g., "Push", "Pull", "Legs"), and the value is an array of exercise objects.

Each exercise object in the workout array MUST have the following keys: "name", "warmupSets", "workingSets", "reps", "rest", "earlySetRPE", "lastSetRPE".

Follow the rules for "warmupSets" and "RPE" from previous instructions. Be intelligent in identifying week ranges and consistent workout names across weeks.`;
}
...
document.getElementById('transcribe-btn').addEventListener('click', async () => {
    ...
    } else if (aiContext.type === 'program_workout') {
         if (!apiKey || !file) { showToast("Please provide an API key and file.", "error"); return; }
        systemPrompt = getProgramWorkoutAiPrompt(aiContext.programName, aiContext.weekLabel, aiContext.workoutName);
    } else { // full_program
        if (!apiKey || !planName || !file) { showToast("Please fill out all AI fields.", "error"); return; }
        if (programs[planName]) { showToast(`Program "${planName}" already exists.`, "error"); return; }
        systemPrompt = getFullProgramAiPrompt();
    }
    ...
    reader.onload = async () => {
        ...
        try {
            ...
            const jsonString = rawText.substring(rawText.indexOf('{'), rawText.lastIndexOf('}') + 1) || rawText.substring(rawText.indexOf('['), rawText.lastIndexOf(']') + 1);
            ...
            if (aiContext.type === 'workout') {
                ...
                programs[aiContext.programName].weeks[aiContext.weekIndex].workouts[aiContext.workoutName] = resultData;
                const weekBlock = document.querySelector(`.program-week-block[data-week-index='${aiContext.weekIndex}']`);
                if(weekBlock) {
                    const newWeekBlock = createWeekBlock(programs[aiContext.programName].weeks[aiContext.weekIndex], aiContext.weekIndex);
                    weekBlock.parentNode.replaceChild(newWeekBlock, weekBlock);
                    newWeekBlock.querySelector('.week-content-collapsible').style.maxHeight = '1000px';
                    newWeekBlock.dataset.state = 'open';
                }
                showToast(`Successfully transcribed "${aiContext.workoutName}"!`, 'success');
            } else { // full_program
                programs[planName] = resultData;
                await saveUserData('programs', programs);
                showToast(`Successfully transcribed program "${planName}"!`, 'success');
            }
...
document.getElementById('open-ai-program-transcribe-btn').addEventListener('click', () => {
    aiContext = { type: 'full_program' };
    document.getElementById('ai-transcribe-title').textContent = 'Transcribe Entire Program';
    document.getElementById('ai-transcribe-description').textContent = 'The AI will attempt to parse the entire file into a new multi-week program.';
    document.getElementById('ai-plan-name-group').style.display = 'block';
    addProgramOptionsOverlay.classList.remove('active');
    aiTranscribeOverlay.classList.add('active');
});
...
// --- NEW PROGRAM EDITOR EVENT DELEGATION ---
document.getElementById('program-editor-body').addEventListener('click', e => {
    ...
    // Accordion logic
    if (e.target.closest('.program-week-header-clickable')) {
        const content = weekBlock.querySelector('.week-content-collapsible');
        if (weekBlock.dataset.state === 'closed') {
            document.querySelectorAll('.program-week-block').forEach(wb => {
                wb.dataset.state = 'closed';
                wb.querySelector('.week-content-collapsible').style.maxHeight = null;
            });
            weekBlock.dataset.state = 'open';
            content.style.maxHeight = content.scrollHeight + "px";
        } else {
            weekBlock.dataset.state = 'closed';
    ...
    if (e.target.closest('.delete-week-btn')) {
        if (confirm(`Are you sure you want to delete "${programs[programName].weeks[weekIndex].weekLabel}"?`)) {
            programs[programName].weeks.splice(weekIndex, 1);
    ...
    if (e.target.closest('.change-duration-btn')) {
        const currentLabel = weekBlock.querySelector('.week-label-title').textContent;
        const startWeek = parseWeekLabel(currentLabel).start;
        const endWeek = prompt(`This block starts at Week ${startWeek}. What week should it end on?`, startWeek);
        if (endWeek && !isNaN(endWeek) && parseInt(endWeek) >= startWeek) {
            const newLabel = parseInt(endWeek) === startWeek ? `Week ${startWeek}` : `Week ${startWeek}-${endWeek}`;
            weekBlock.querySelector('.week-label-title').textContent = newLabel;
            programs[programName].weeks[weekIndex].weekLabel = newLabel;
        } else if (endWeek !== null) {
            showToast("Invalid end week.", "error");
        }
    }

    // Add workout to week logic
    if (e.target.closest('.add-workout-to-week-btn')) {
        programEditorContext = { programName, weekIndex };
        document.getElementById('add-workout-to-week-main-options').style.display = 'block';
        document.getElementById('existing-workouts-selection').style.display = 'none';
        addWorkoutToWeekOverlay.classList.add('active');
    }
});

// --- NEW "ADD WORKOUT TO WEEK" MODAL LISTENERS ---
document.getElementById('close-add-workout-to-week-btn').addEventListener('click', () => addWorkoutToWeekOverlay.classList.remove('active'));
document.getElementById('back-to-add-workout-options-btn').addEventListener('click', () => {
    document.getElementById('add-workout-to-week-main-options').style.display = 'block';
    document.getElementById('existing-workouts-selection').style.display = 'none';
});

document.getElementById('add-new-workout-to-program-btn').addEventListener('click', () => {
    const { programName, weekIndex } = programEditorContext;
    const workoutName = prompt("Enter name for this new workout (e.g., Push, Pull):");
    if (workoutName) {
        if (!programs[programName].weeks[weekIndex].workouts) {
            programs[programName].weeks[weekIndex].workouts = {};
        }
        programs[programName].weeks[weekIndex].workouts[workoutName] = [{ name: "", warmupSets: "", workingSets: "", reps: "", rest: "", earlySetRPE: "", lastSetRPE: "" }];
        
        addWorkoutToWeekOverlay.classList.remove('active');
        openProgramEditor(programName);
        setTimeout(() => {
...
document.getElementById('add-existing-workout-to-program-btn').addEventListener('click', () => {
    const select = document.getElementById('existing-workout-select');
    select.innerHTML = '<option value="">-- Select a workout --</option>';
    const standaloneWorkouts = Object.keys(workouts).filter(name => !activeProgram || !name.startsWith(activeProgram.programName));
    standaloneWorkouts.forEach(name => {
        select.innerHTML += `<option value="${name}">${name}</option>`;
    });
    document.getElementById('add-workout-to-week-main-options').style.display = 'none';
    document.getElementById('existing-workouts-selection').style.display = 'block';
});

document.getElementById('confirm-add-existing-btn').addEventListener('click', () => {
    const { programName, weekIndex } = programEditorContext;
    const selectedWorkoutName = document.getElementById('existing-workout-select').value;
    if (!selectedWorkoutName) {
        showToast("Please select a workout.", "error");
        return;
    }
    
    const workoutDataCopy = JSON.parse(JSON.stringify(workouts[selectedWorkoutName]));
    if (!programs[programName].weeks[weekIndex].workouts) {
        programs[programName].weeks[weekIndex].workouts = {};
    }
    programs[programName].weeks[weekIndex].workouts[selectedWorkoutName] = workoutDataCopy;

    addWorkoutToWeekOverlay.classList.remove('active');
    const weekBlock = document.querySelector(`.program-week-block[data-week-index='${weekIndex}']`);
    if(weekBlock) {
        const newWeekBlock = createWeekBlock(programs[programName].weeks[weekIndex], weekIndex);
        weekBlock.parentNode.replaceChild(newWeekBlock, weekBlock);
        newWeekBlock.querySelector('.week-content-collapsible').style.maxHeight = '1000px';
        newWeekBlock.dataset.state = 'open';
    }
    showToast(`Copied "${selectedWorkoutName}" to the program.`, "success");
});

document.getElementById('ai-transcribe-to-program-btn').addEventListener('click', () => {
    const { programName, weekIndex } = programEditorContext;
    const workoutName = prompt("What is the name of the workout in the file? (e.g., Push Day)");
